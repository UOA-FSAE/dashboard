...
//    Vehicle_Data the_car_buffalo;
//    resetDataStructure(&the_car);
//
//    // Fucking with the ram
//    HAL_SDRAM_StateTypeDef ram_status = HAL_SDRAM_GetState(&hsdram1);
//
    volatile uint32_t *ram_address = (uint32_t *) 0x6FFFFFFF;
//    memcpy(ram_address, &the_car, sizeof(Vehicle_Data));
//    memcpy(&the_car_bagalo, ram_address, sizeof(Vehicle_Data));
//
//    memset(&the_car,0,sizeof(Vehicle_Data));
//    memset(ram_address,0,sizeof(Vehicle_Data));
//    memset(&the_car_buffalo,0,sizeof(Vehicle_Data));

//    uint16_t test_buffer[100] = {0};
    uint32_t test_buffer_two_point_oh[100] = {0};

//    HAL_SDRAM_Write_16b(&hsdram1, (uint32_t *)ram_address, (uint16_t *)test_buffer, 100);
//    HAL_SDRAM_Read_16b(&hsdram1, (uint32_t *)ram_address, (uint16_t *)test_buffer_two_point_oh, 100);
    for (uint32_t i = 0; i < 100; i++) {
        ram_address[i] = i;
    }

    for (uint32_t i = 0; i < 100; i++) {
        test_buffer_two_point_oh[0] = ram_address[i];
    }
